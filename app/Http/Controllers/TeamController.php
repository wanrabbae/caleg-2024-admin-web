...
class TeamController extends Controller
{
    public function index()
    {
        ...
        return view('relawan.index', [
            ...
            'data' => auth('web')->check()
                ? Relawan::with(['caleg', 'desa.kecamatan'])
                    ->search(request('search'))
                    ->paginate(request('paginate') ?? 10)
                    ->withQueryString()
                : Relawan::with(['caleg', 'desa.kecamatan'])
                    ->where('id_caleg', auth()->user()->id_caleg)
                    ->search(request('search'))
                    ->paginate(request('paginate') ?? 10)
                    ->withQueryString(),
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('getTps') && $request->getTps) {
            ...
            $relawanTps = Relawan::where('id_desa', $desa->id_desa)
                ->where('id_caleg', $desa->id_caleg)
                ->get(['tps']);
            $checkTps = [];
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('jabatan')) {
            ...
            if (
                $request->jabatan == 2 &&
                gettype(
                    Relawan::with('desa.kecamatan')
                        ->get()
                        ->filter(function ($value, $i) use ($relawan) {
    ...
    public function upline($id)
    {
        ...
        return view('relawan.upline', [
            'title' => 'Upline ' . Relawan::find($id)->nama_relawan,
            'data' => Relawan::with(['caleg', 'desa'])
                ->where('id_caleg', Relawan::find($id)->id_caleg)
                ->where('upline', $id)
                ->search(request('search'))
                ->paginate(request('paginate') ?? 10)
                ->withQueryString(),
